...
bool exit_in_progress = false, command_aborted = false, display_aborted = false;
...
deque<MathStructure*> inhistory_value;

int current_inhistory_index = 0;
int history_index = 0;
...
void update_expression_icons(bool stop_icon = FALSE) {
	if(stop_icon) {
		...
		gtk_entry_set_icon_from_icon_name(GTK_ENTRY(expression), GTK_ENTRY_ICON_PRIMARY, "process-stop");
		gtk_entry_set_icon_sensitive(GTK_ENTRY(expression), GTK_ENTRY_ICON_PRIMARY, TRUE);
	} else {
		gtk_entry_set_icon_from_icon_name(GTK_ENTRY(expression), GTK_ENTRY_ICON_PRIMARY, "system-run-symbolic");
		if(rpn_mode) {
			gtk_entry_set_icon_tooltip_text(GTK_ENTRY(expression), GTK_ENTRY_ICON_PRIMARY, _("Calculate expression and add to stack"));
		} else {
			gtk_entry_set_icon_tooltip_text(GTK_ENTRY(expression), GTK_ENTRY_ICON_PRIMARY, _("Calculate expression"));
		}
	}
	if(stop_icon || gtk_entry_get_text_length(GTK_ENTRY(expression)) == 0) {
		if(!stop_icon) {
			gtk_entry_set_icon_sensitive(GTK_ENTRY(expression), GTK_ENTRY_ICON_PRIMARY, FALSE);
		}
		gtk_entry_set_icon_from_icon_name(GTK_ENTRY(expression), GTK_ENTRY_ICON_SECONDARY, NULL);
	} else {
		gtk_entry_set_icon_from_icon_name(GTK_ENTRY(expression), GTK_ENTRY_ICON_SECONDARY, "edit-clear-symbolic");
		gtk_entry_set_icon_tooltip_text(GTK_ENTRY(expression), GTK_ENTRY_ICON_SECONDARY, _("Clear expression"));
	}
}
...
void set_unicode_buttons() {
	if(printops.use_unicode_signs) {
		if(can_display_unicode_string_function(SIGN_MINUS, (void*) gtk_builder_get_object(main_builder, "label_sub"))) gtk_label_set_text(GTK_LABEL(gtk_builder_get_object(main_builder, "label_sub")), SIGN_MINUS);
		else gtk_label_set_text(GTK_LABEL(gtk_builder_get_object(main_builder, "label_sub")), MINUS);
		if(can_display_unicode_string_function(SIGN_PLUS, (void*) gtk_builder_get_object(main_builder, "label_add"))) gtk_label_set_text(GTK_LABEL(gtk_builder_get_object(main_builder, "label_add")), SIGN_PLUS);
...
void display_errors(int *history_index_p = NULL, GtkWidget *win = NULL, int *inhistory_index = NULL, int type = 0) {
	...
	while(true) {
		...
		if((mtype == MESSAGE_ERROR || mtype == MESSAGE_WARNING) && history_index_p && inhistory_index) {
			if(mtype == MESSAGE_ERROR) {
				...
				inhistory_value.insert(inhistory_value.begin() + *inhistory_index, NULL);
				string history_str = "<span foreground=\"red\">- ";
				history_str += CALCULATOR->message()->message();
				history_str += "</span>";
				(*history_index_p)++;
				gtk_list_store_insert_with_values(historystore, &history_iter, *history_index_p, 0, history_str.c_str(), 1, *inhistory_index, -1);
	...
	if(!str.empty()) {
		if(type == 1) {
			...
			gtk_widget_show(GTK_WIDGET(gtk_builder_get_object(main_builder, "message_tooltip_icon")));
			if(first_error) {
...
cairo_surface_t *get_left_parenthesis(gint arc_w, gint arc_h, int) {
...
cairo_surface_t *get_right_parenthesis(gint arc_w, gint arc_h, int) {
	gint par_h = 0, par_w = 0;
	PangoLayout *layout = gtk_widget_create_pango_layout(resultview, NULL);
	pango_layout_set_markup(layout, "<span font=\"100\">)</span>", -1);
	pango_layout_get_pixel_size(layout, &par_w, &par_h);
...
cairo_surface_t *draw_structure(MathStructure &m, PrintOptions po, InternalPrintStruct ips, gint *point_central, int scaledown) {
	...
	switch(m.type()) {
		...
		case STRUCT_ADDITION: {
			...
			for(size_t i = 0; i < m.size(); i++) {
				...
				if(m[i].type() == STRUCT_NEGATE && i > 0) {
					...
					surface_terms.push_back(draw_structure(m[i][0], po, ips_n, &hetmp, scaledown));
				} else {
		...
		case STRUCT_NEGATE: {
			...
			cairo_surface_t *surface_arg = draw_structure(m[0], po, ips_n, &ctmp, scaledown);
			if(!surface_arg) {
				g_object_unref(layout_minus);
				return NULL;
			}
		...
		case STRUCT_DIVISION: {
			...
			if(m.type() == STRUCT_DIVISION) {
				...
				num_surface = draw_structure(m[0], po, ips_n, &num_dh, scaledown);
				if(!num_surface) {
					return NULL;
				}
				...
				ips_n.wrap = false;
				surface_one = draw_structure(onestruct, po, ips_n, NULL, scaledown);
				if(!surface_one) {
					return NULL;
				}
			...
			if(m.type() == STRUCT_DIVISION) {
				...
				den_surface = draw_structure(m[1], po, ips_n, &den_dh, scaledown);
			} else {
				ips_n.wrap = m[0].needsParenthesis(po, ips_n, m, 2, flat, ips.power_depth > 0);
				den_surface = draw_structure(m[0], po, ips_n, &den_dh, scaledown);
			}
			if(!den_surface) {
		...
		case STRUCT_POWER: {
			...
			CALCULATE_SPACE_W
			ips_n.wrap = m[0].needsParenthesis(po, ips_n, m, 1, ips.division_depth > 0 || ips.power_depth > 0, ips.power_depth > 0);
			cairo_surface_t *surface_base = draw_structure(m[0], po, ips_n, &central_point, scaledown);
			if(!surface_base) {
				return NULL;
			}
			...
			cairo_surface_t *surface_exp = draw_structure(m[1], po2, ips_n, &ctmp, scaledown);
			if(!surface_exp) {
			...
			if(layout_power) {
				...
				pango_cairo_show_layout(cr, layout_power);
				w += power_w;
			}
			gdk_cairo_set_source_rgba(cr, &rgba);
		...
		case STRUCT_VECTOR: {
			...
			w = 0;
			cairo_set_source_surface(cr, get_left_parenthesis(arc_w, arc_h, scaledown), w, uh - arc_h / 2 - arc_h % 2);
			cairo_paint(cr);
			w += arc_w;
			if(m.size() == 0) w += 2;
			...
			cairo_set_source_surface(cr, get_right_parenthesis(arc_w, arc_h, scaledown), w, uh - arc_h / 2 - arc_h % 2);
			cairo_paint(cr);
	...
	if(ips.wrap && surface) {
		...
		gdk_cairo_set_source_rgba(cr, &rgba);
		w = 0;
		cairo_set_source_surface(cr, get_left_parenthesis(arc_base_w, arc_base_h, scaledown), w, h - arc_base_h);
		cairo_paint(cr);
		w += arc_base_w;
		...
		w += base_w;
		cairo_set_source_surface(cr, get_right_parenthesis(arc_base_w, arc_base_h, scaledown), w, h - arc_base_h);
		cairo_paint(cr);
	}
...
void clearresult() {
	...
	if(gtk_widget_get_visible(GTK_WIDGET(gtk_builder_get_object(main_builder, "message_tooltip_icon")))) {
		gtk_widget_hide(GTK_WIDGET(gtk_builder_get_object(main_builder, "message_tooltip_icon")));
	}
	if(surface_result) {
...
void *view_proc(void *pipe) {
	...
	while(true) {
		...
		void *x = NULL;
		int scale_tmp = 0;
		fread(&scale_tmp, sizeof(int), 1, view_pipe);
...
void setResult(Prefix *prefix, bool update_history, bool update_parse, bool force, string transformation, size_t stack_index, bool register_moved) {
	...
	if(update_history) {
		if(update_parse || register_moved) {
			if(register_moved) {
				...
				inhistory.push_back("");
				inhistory_value.push_back(NULL);
			} else {
			...
			gtk_list_store_insert_with_values(historystore, &history_iter, 0, 0, result_text.c_str(), 1, inhistory.size() - 1, 2, i2s(nr_of_new_expressions).c_str(), -1);
			gtk_list_store_insert_with_values(historystore, NULL, 1, 1, -1, 3, TRUE, 4, 2.0, -1);
			history_index = 0;			
			...
			if(!transformation.empty()) {
				...
				gtk_list_store_insert_with_values(historystore, &history_iter, -1, 0, history_str.c_str(), 1, -1, -1);
	...
	if(stack_index != 0) {
		...
		if(update_parse) {
			...
			parsed_mstruct->ref();
			inhistory_value.insert(inhistory_value.begin() + inhistory_index, parsed_mstruct);
		...
		history_index++;
		gtk_list_store_insert_with_values(historystore, &history_iter, history_index, 0, (str + result_text + "</span>").c_str(), 1, inhistory_index, -1);
		...
		mstruct->ref();
		inhistory_value.insert(inhistory_value.begin() + inhistory_index, mstruct);
...
void insert_function(MathFunction *f, GtkWidget *parent = NULL) {
	...
	dialog = gtk_dialog_new_with_buttons(f_title.c_str(), GTK_WINDOW(parent), GTK_DIALOG_DESTROY_WITH_PARENT, _("_Cancel"), GTK_RESPONSE_REJECT, NULL);
	...
	for(int i = 0; i < args; i++) {
		...
		if(type_label[i]) {
			gtk_widget_set_hexpand(type_label[i], TRUE);
	...
	if(!f->description().empty()) {
		GtkWidget *descr_frame = gtk_frame_new(NULL);
			...
			switch(u->subtype()) {
				case SUBTYPE_ALIAS_UNIT: {
					...
					if(!u->isBuiltin()) {
						...
						au->setInverseExpression(CALCULATOR->unlocalizeExpression(gtk_entry_get_text(GTK_ENTRY(gtk_builder_get_object(unitedit_builder, "unit_edit_entry_reversed"))), evalops.parse_options));
...
void edit_argument(Argument *arg) {
	...
	switch(arg->type()) {
		case ARGUMENT_TYPE_NUMBER: {
			...
			gtk_spin_button_set_numeric(GTK_SPIN_BUTTON(gtk_builder_get_object(argumentrules_builder, "argument_rules_spinbutton_min")), FALSE);
			if(farg->min()) {
				PrintOptions po;
				po.number_fraction_format = FRACTION_DECIMAL_EXACT;
				gtk_entry_set_text(GTK_ENTRY(gtk_builder_get_object(argumentrules_builder, "argument_rules_spinbutton_min")), farg->min()->print(po).c_str());
			} else {
			...
			gtk_spin_button_set_numeric(GTK_SPIN_BUTTON(gtk_builder_get_object(argumentrules_builder, "argument_rules_spinbutton_max")), FALSE);
			if(farg->max()) {
				PrintOptions po;
				po.number_fraction_format = FRACTION_DECIMAL_EXACT;
				gtk_entry_set_text(GTK_ENTRY(gtk_builder_get_object(argumentrules_builder, "argument_rules_spinbutton_max")), farg->max()->print(po).c_str());
			} else {
		...
		case ARGUMENT_TYPE_INTEGER: {
			...
			gtk_spin_button_set_numeric(GTK_SPIN_BUTTON(gtk_builder_get_object(argumentrules_builder, "argument_rules_spinbutton_min")), TRUE);
			if(iarg->min()) {
			...
			gtk_spin_button_set_numeric(GTK_SPIN_BUTTON(gtk_builder_get_object(argumentrules_builder, "argument_rules_spinbutton_max")), TRUE);
			if(iarg->max()) {
	...
	if(gtk_dialog_run(GTK_DIALOG(dialog)) == GTK_RESPONSE_OK) {
		...
		if(arg->type() == ARGUMENT_TYPE_NUMBER) {
			...
			if(gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(gtk_builder_get_object(argumentrules_builder, "argument_rules_checkbutton_enable_min")))) {
				MathStructure mstruct2 = CALCULATOR->calculate(CALCULATOR->unlocalizeExpression(gtk_entry_get_text(GTK_ENTRY(gtk_builder_get_object(argumentrules_builder, "argument_rules_spinbutton_min"))), evalops.parse_options));
			...
			if(gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(gtk_builder_get_object(argumentrules_builder, "argument_rules_checkbutton_enable_max")))) {
				MathStructure mstruct2 = CALCULATOR->calculate(CALCULATOR->unlocalizeExpression(gtk_entry_get_text(GTK_ENTRY(gtk_builder_get_object(argumentrules_builder, "argument_rules_spinbutton_max"))), evalops.parse_options));
...
void edit_unknown(const char *category, Variable *var, GtkWidget *win) {
	...
	if(v) {
		...
		gtk_widget_set_sensitive(GTK_WIDGET(gtk_builder_get_object(unknownedit_builder, "unknown_edit_combobix_type")), !v->isBuiltin());
		...
		if(v->assumptions()) {
			...
			gtk_combo_box_set_active(GTK_COMBO_BOX(gtk_builder_get_object(unknownedit_builder, "unknown_edit_combobox_type")), v->assumptions()->type());
			...
			gtk_combo_box_set_active(GTK_COMBO_BOX(gtk_builder_get_object(unknownedit_builder, "unknown_edit_combobox_type")), CALCULATOR->defaultAssumptions()->type());
		...
		if(v) {
			if(!v->isBuiltin()) {
				if(gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(gtk_builder_get_object(unknownedit_builder, "unknown_edit_checkbutton_custom_assumptions")))) {
					...
					v->assumptions()->setType((AssumptionType) gtk_combo_box_get_active(GTK_COMBO_BOX(gtk_builder_get_object(unknownedit_builder, "unknown_edit_combobox_type"))));
...
void edit_variable(const char *category, Variable *var, MathStructure *mstruct_, GtkWidget *win) {
	...
	if(v) {
		...
		gtk_entry_set_text(GTK_ENTRY(gtk_builder_get_object(variableedit_builder, "variable_edit_entry_value")), get_value_string(v->get(), false, NULL).c_str());
...
void on_history_populate_popup(GtkTextView*, GtkMenu *sub, gpointer) {
	GtkWidget *item;
	MENU_SEPARATOR
	item = gtk_menu_item_new_with_mnemonic(_("_Clear"));
	gtk_widget_show(item);
	g_signal_connect(G_OBJECT(item), "activate", G_CALLBACK(on_popup_menu_item_clear_history_active), NULL);
	gtk_menu_shell_append(GTK_MENU_SHELL(sub), item);
		...
		while(true) {
			...
			if((i = stmp.find_first_of("=")) != string::npos) {
				...
				if(svar == "version") {
					...
					inhistory_value.push_front(NULL);
...
gboolean completion_match_func(GtkEntryCompletion*, const gchar*, GtkTreeIter *iter, gpointer) {
	...
	if((editing_to_expression || !evalops.parse_options.functions_enabled) && item->type() == TYPE_FUNCTION) return FALSE;
	if((editing_to_expression || !evalops.parse_options.variables_enabled) && item->type() == TYPE_VARIABLE) return FALSE;
...
gboolean on_expression_button_press_event(GtkWidget*, GdkEventButton *event, gpointer) {
...
void on_expression_icon_release(GtkEntry *entry, GtkEntryIconPosition icon_pos, GdkEvent*, gpointer) {
	switch(icon_pos) {
...
void on_expression_changed(GtkEditable *w, gpointer) {
	...
	gtk_entry_set_icon_sensitive(GTK_ENTRY(expression), GTK_ENTRY_ICON_PRIMARY, l > 0);
	gtk_entry_set_icon_from_icon_name(GTK_ENTRY(expression), GTK_ENTRY_ICON_SECONDARY, l > 0 ? "edit-clear-symbolic" : NULL);
	gtk_entry_set_icon_tooltip_text(GTK_ENTRY(expression), GTK_ENTRY_ICON_SECONDARY, l > 0 ? _("Clear expression") : NULL);
...
void on_historyview_selection_changed(GtkTreeSelection *select, gpointer) {
	...
	gtk_widget_set_sensitive(GTK_WIDGET(gtk_builder_get_object(main_builder, "button_history_paste")), row_count == 1);
}
void on_button_history_paste_clicked(GtkButton*, gpointer) {	
	GtkTreeModel *model;
	GtkTreeIter iter;
	GList *selected_rows;
	gint index = -1;
...
void on_menu_item_plot_functions_activate(GtkMenuItem*, gpointer) {
	...
	gtk_window_set_transient_for(GTK_WINDOW(dialog), GTK_WINDOW(gtk_builder_get_object(main_builder, "main_window")));
	gtk_entry_set_text(GTK_ENTRY(gtk_builder_get_object(plot_builder, "plot_entry_expression")), gtk_entry_get_text(GTK_ENTRY(expression)));
	if(!gtk_widget_get_visible(dialog)) {
...
void on_menu_item_always_exact_activate(GtkMenuItem *w, gpointer) {
	if(!gtk_check_menu_item_get_active(GTK_CHECK_MENU_ITEM(w))) return;
	evalops.approximation = APPROXIMATION_EXACT;
	GtkWidget *w_exact = GTK_WIDGET(gtk_builder_get_object(main_builder, "button_exact"));
...
void on_menu_item_fraction_decimal_activate(GtkMenuItem *w, gpointer) {
	...
	GtkWidget *w_fraction = GTK_WIDGET(gtk_builder_get_object(main_builder, "button_fraction"));
...
void on_menu_item_save_image_activate(GtkMenuItem*, gpointer) {
	...
	if(gtk_dialog_run(GTK_DIALOG(d)) == GTK_RESPONSE_ACCEPT) {
		cairo_surface_write_to_png(surface_result, gtk_file_chooser_get_filename(GTK_FILE_CHOOSER(d)));
	}
	gtk_widget_destroy(d);
}
...
void on_unknown_edit_combobox_type_changed(GtkComboBox *om, gpointer) {	
	if((AssumptionType) gtk_combo_box_get_active(om) <= ASSUMPTION_TYPE_COMPLEX && (AssumptionSign) gtk_combo_box_get_active(GTK_COMBO_BOX(gtk_builder_get_object(unknownedit_builder, "unknown_edit_combobox_sign"))) != ASSUMPTION_SIGN_NONZERO) {
		g_signal_handlers_block_matched((gpointer) gtk_builder_get_object(unknownedit_builder, "unknown_edit_combobox_sign"), G_SIGNAL_MATCH_FUNC, 0, 0, NULL, (gpointer) on_unknown_edit_combobox_sign_changed, NULL);
...
void on_unknown_edit_combobox_sign_changed(GtkComboBox *om, gpointer) {	
	if((AssumptionSign) gtk_combo_box_get_active(om) != ASSUMPTION_SIGN_NONZERO && (AssumptionSign) gtk_combo_box_get_active(om) != ASSUMPTION_SIGN_UNKNOWN && (AssumptionType) gtk_combo_box_get_active(GTK_COMBO_BOX(gtk_builder_get_object(unknownedit_builder, "unknown_edit_combobox_type"))) <= ASSUMPTION_TYPE_COMPLEX) {
		g_signal_handlers_block_matched((gpointer) gtk_builder_get_object(unknownedit_builder, "unknown_edit_combobox_type"), G_SIGNAL_MATCH_FUNC, 0, 0, NULL, (gpointer) on_unknown_edit_combobox_type_changed, NULL);
		gtk_combo_box_set_active(GTK_COMBO_BOX(gtk_builder_get_object(unknownedit_builder, "unknown_edit_combobox_type")), ASSUMPTION_TYPE_REAL);	
		g_signal_handlers_unblock_matched((gpointer) gtk_builder_get_object(unknownedit_builder, "unknown_edit_combobox_type"), G_SIGNAL_MATCH_FUNC, 0, 0, NULL, (gpointer) on_unknown_edit_combobox_type_changed, NULL);
	}
}

gboolean on_key_press_event(GtkWidget *o, GdkEventKey *event, gpointer) {
	if(gtk_widget_has_focus(expression) || b_editing_stack) return FALSE;
...
void on_csv_import_button_file_clicked(GtkButton*, gpointer) {
	...
	if(gtk_dialog_run(GTK_DIALOG(d)) == GTK_RESPONSE_ACCEPT) {
		gtk_entry_set_text(GTK_ENTRY(gtk_builder_get_object(csvimport_builder, "csv_import_entry_file")), gtk_file_chooser_get_filename(GTK_FILE_CHOOSER(d)));
	}
	gtk_widget_destroy(d);
}
