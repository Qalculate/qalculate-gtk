...
#define MENU_ITEM_WITH_POINTER_AND_FLAG(x,y,z)	{GtkWidget *box = gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 6); string imagefile = "/qalculate-gtk/flags/"; imagefile += z->referenceName(); imagefile += ".png"; if(g_resources_get_info(imagefile.c_str(), G_RESOURCE_LOOKUP_FLAGS_NONE, NULL, NULL, NULL)) gtk_container_add(GTK_CONTAINER(box), gtk_image_new_from_resource(imagefile.c_str())); gtk_container_add(GTK_CONTAINER(box), gtk_label_new(x)); item = gtk_menu_item_new(); gtk_container_add(GTK_CONTAINER(item), box); gtk_widget_show_all(item); g_signal_connect (G_OBJECT (item), "activate", G_CALLBACK(y), (gpointer) z); gtk_menu_shell_append(GTK_MENU_SHELL(sub), item);}
...
#define ALWAYS_EXACT_INDEX		0
#define TRY_EXACT_INDEX			2
#define APPROXIMATE_INDEX		3
#define INTERVAL_ARITHMETIC_INDEX	1
